  ...
  for (int_fast64_t x = 0; x < LAKE_MAX_DIAMETER; ++x) {
    for (int_fast64_t z = 0; z < LAKE_MAX_DIAMETER; ++z) {
      for (int_fast32_t y = 0; y < LAKE_MAX_DIAMETER; ++y) {
        ...
        if ((blockType == 3 && (blockAboveType == 17 || blockAboveType == 162)) || blockType == 17 ||
            blockType == 162) {
          continue;
        }
...
bool Lake::IsLakeBlock(std::vector<int_fast64_t> &lakeMap, int_fast64_t x, int_fast32_t y, int_fast64_t z) {
  return std::find(lakeMap.begin(), lakeMap.end(), (x * LAKE_MAX_DIAMETER + z) * LAKE_MAX_HEIGHT + y)
      != lakeMap.end();
}

void Lake::SetLakeBlock(std::vector<int_fast64_t> &lakeMap, int_fast64_t x, int_fast32_t y, int_fast64_t z) {
  lakeMap.emplace_back((x * LAKE_MAX_DIAMETER + z) * LAKE_MAX_HEIGHT + y);
}

bool Lake::CanPlace(std::vector<int_fast64_t> &lakeMap,
                    ChunkManager world,
                    int_fast64_t sourceX,
                    int_fast32_t sourceY,
                    int_fast64_t sourceZ) {
