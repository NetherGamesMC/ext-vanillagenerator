...
uint_fast64_t Math::morton2d_encode(int64_t x, int64_t y) {
  return libmorton::morton2D_64_encode(x, y);
}

void Math::morton2d_decode(int_fast64_t chunkCord, int_fast64_t &chunkX, int_fast64_t &chunkZ) {
...
uint_fast64_t Math::morton3d_encode(int_fast64_t x, int_fast32_t y, int_fast64_t z) {
...
void Math::morton3d_decode(uint_fast64_t chunkCord, int_fast32_t &rx, int_fast32_t &ry, int_fast32_t &rz) {
  const size_t SHIFT = (sizeof(zend_long) * 8) - 32;
  ...
  rx = static_cast<int_fast32_t>(((baseX & BLOCKHASH_XZ_MASK) | extraX) << BLOCKHASH_XZ_SIGN_SHIFT
                                                                        >> BLOCKHASH_XZ_SIGN_SHIFT);
  ry = static_cast<int_fast32_t>((baseY & BLOCKHASH_Y_MASK) - BLOCKHASH_Y_OFFSET);
  rz = static_cast<int_fast32_t>(((baseZ & BLOCKHASH_XZ_MASK) | extraZ) << BLOCKHASH_XZ_SIGN_SHIFT
                                                                        >> BLOCKHASH_XZ_SIGN_SHIFT);
}